...
def digit_root(number: int) -> int:
    """Returns the digit root of an input number -- (the recursive sum of each
    digit until a single digit is left)

    args:
        number -- input number for which the digit root is calculated
    """
    string_int = str(number)
    if len(string_int) == 1:
        return number
    else:
        root = reduce(lambda sum, char: sum + int(char), string_int, 0)
